...
TIME_DELTA = 3 | сдвиг метки времени в часах (для учета часового пояса)
8. Запустить скрипт.
...
10. Подключиться к КС с помощью OPC UA Expert (или другим клиентом OPC UA). Проконтролировать обновление сигналов.
...
## Примечание
Эликонт-КС использует время в формате [unixtime](https://ru.wikipedia.org/wiki/Unix-%D0%B2%D1%80%D0%B5%D0%BC%D1%8F), но дополнительно учитываются миллисекунды.

Для отладки можно использовать [онлайн-конвертор](https://currentmillis.com/). Там же можно изучить примеры кода для разных языков.
...
>pyinstaller --onefile --hidden-import win32timezone opcda_client.py

В результате компиляции получается файл /dist/opcda_client.exe
